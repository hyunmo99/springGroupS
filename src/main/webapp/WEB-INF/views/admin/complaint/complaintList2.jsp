...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="${ctp}/css/rinkOrange.css">
	<script>
		'use strict'
		//신고 내역 모달로 확인
  	...
  	<c:forEach var="vo" items="${vos}" varStatus="st">
		 	<tr>
		 		...
		 		<c:set var="content" value="${fn:replace(fn:replace(vo.content, CRLF, '<br/>'),LF, '<br/>')}"/>
		 			<%-- <a href="#" onclick="modalCheck('${vo.mid}','${vo.title}','${content}','${vo.nickName}')" data-bs-toggle="modal" data-bs-target="#myModal" class="">${vo.title}</a> --%>
		 			<a href="#" onclick="modalCheck('${vo.mid}','${vo.title}','${vo.nickName}')" data-bs-toggle="modal" data-bs-target="#myModal" class="">${vo.title}</a>
		 		</td>		
		 		<td>
		 		...
		 		<td>
		 			<c:if test="${vo.progress =='신고접수'}">
						...
						<a href="javascript:complaintProcess('${vo.idx}','${vo.part}','${vo.partIdx}','H')" class="badge bg-warning">감추기</a>
