        ...
        <td class="text-start">
          <c:if test="${vo.openSw == 'NO'}">
            ...
            <c:if test="${sMid == vo.mid || sAdmin == 'adminOK'}">
		          <a href="boardContent?idx=${vo.idx}&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}&boardFlag=search" class="text-decoration-none text-dark link-primary">
          ...
          <c:if test="${vo.openSw != 'NO'}">
	          <a href="boardContent?idx=${vo.idx}&boardFlag=search&search=${pageVO.search}&searchString=${pageVO.searchString}" class="text-decoration-none text-dark link-primary">${vo.title}</a>
	          <c:if test="${vo.hour_diff <= 24}"><img src="${ctp}/images/new.gif" /></c:if>
          </c:if>
        </td>
        <td>${vo.nickName}</td>
...
<!-- 블록페이지 시작 -->
	...
	<div class="pagination justify-content-center">
	  ...
	  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize)+pageVO.blockSize}" varStatus="st">
	  	<c:if test="${i <= pageVO.totPage && i == pageVO.pag}"><a href="BoardList.bo?pag=${i}&pageSize=${pageVO.pageSize}" class="page-item page-link active text-decoration-none bg-secondary border-secondary">${i}</a></c:if>
	  	<c:if test="${i <= pageVO.totPage && i != pageVO.pag}"><a href="BoardList.bo?pag=${i}&pageSize=${pageVO.pageSize}" class="page-item page-link text-decoration-none text-dark link-primary">${i}</a></c:if>
	  </c:forEach>
	  <c:if test="${pageVO.curBlock < pageVO.lastBlock}"><a href="BoardList.bo?pag=${(pageVO.curBlock+1)*pageVO.blockSize + 1}&pageSize=${pageVO.pageSize}" class="page-item page-link text-decoration-none text-dark link-primary">다음블록</a></c:if>
	  <c:if test="${pageVO.pag < pageVO.totPage}"><a href="BoardList.bo?pag=${pageVO.totPage}&pageSize=${pageVO.pageSize}" class="page-item page-link text-decoration-none text-dark link-primary">마지막페이지</a></c:if>
	</div>
<!-- 블록페이지 끝 -->
<br/>
<!-- 검색기 시작 -->
